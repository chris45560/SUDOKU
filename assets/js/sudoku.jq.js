...
/**
 * Ecriture du résultat du sudoku dans la grille
 */
...
var resolveSudoku = function() {
	...
	if (sBase.length >= 81) {
		var solver = sudoku_solver();
		var aResult = solver(sBase,1); // On ne prend qu'un seul résultat
		if (typeof aResult[0] != 'undefined') {
			// On complète la grille
...
$.fn.createSudokuGrid = function(options) {
    ...
    // Action de contrôle sur les champs
...
/**
 * Contrôle la validation d'un champ
 */
 $.fn.controlFieldInput = function() {
 	/**
 	 * La méthode de contrôle de la validité d'un bloc
 	 */
 	var zoneIsCorrect = function(sType, oCurrentObj) {
		...
		// De base on considère qu'il n'y a pas d'erreur
		objCible.removeClass('error_'+sType);

		// On boucle sur toutes les cibles
		$(objCible).each(function(){
			// Les cases peuvent être vides
			if ($(this).val() != '') {
				// On ne vérifie pas la case elle même
				if ($(this).val() == iCurrentValue
					&&
					oCurrentObj.attr("id") != $(this).attr("id")
				) {
					// On marque l'erreur sur tout le block
					objCible.addClass('error_'+sType);
					return; // On stoppe s'il y a déjà une erreur
	...
	return this.each(function() {
		...
		$(this).on("keyup", function(event) {
			...
			// On boucle sur la ligne complète
			zoneIsCorrect('row', $(this));
			// On boucle sur la colonne complète
